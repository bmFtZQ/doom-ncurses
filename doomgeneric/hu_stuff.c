...
boolean HU_Responder(event_t *ev)
{
    ...
    if (ev->data1 == DG_KEY_RSHIFT)
    {
        return false;
    }
    else if (ev->data1 == DG_KEY_RALT || ev->data1 == DG_KEY_LALT)
    {
    ...
    else
    {
        ...
        if (altdown)
        {
            ...
            HU_queueChatChar(DG_KEY_ENTER); // DEBUG!!!

            // send the macro message
            while (*macromessage)
                HU_queueChatChar(*macromessage++);
            HU_queueChatChar(DG_KEY_ENTER);
